...
sudo "$VENV_PATH/bin/pip" install --upgrade pip || { echo "ERROR: Falló la actualización de pip en el entorno virtual."; exit 1; }
sudo "$VENV_PATH/bin/pip" install -r "$BACKEND_PROD_DIR/requirements.txt" || { echo "ERROR: Falló la instalación de dependencias de Python para la API."; exit 1; }
